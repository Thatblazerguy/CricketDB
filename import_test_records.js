...
const content = fs.readFileSync(path.join(__dirname, 'test_records.csv'), 'utf8');
...
db.transaction(() => {
  for (const row of records) {
    ...
    const existingStat = checkStat.get(playerId, 'Test');
    if (existingStat) {
      updateStat.run(matches, runs, average, strikeRate, hundreds, fifties, wickets, bowlAvg, existingStat.id);
    } else {
      insertStat.run(playerId, 'Test', matches, runs, average, strikeRate, hundreds, fifties, wickets, bowlAvg);
    }
  }
})();

console.log("Successfully imported Test records!");

...
const content = fs.readFileSync(path.join(__dirname, 'odi_records.csv'), 'utf8');
...
db.transaction(() => {
  for (const row of records) {
    ...
    const existingStat = checkStat.get(playerId, 'ODI');
    if (existingStat) {
      updateStat.run(matches, runs, average, strikeRate, hundreds, fifties, wickets, bowlAvg, existingStat.id);
    } else {
      insertStat.run(playerId, 'ODI', matches, runs, average, strikeRate, hundreds, fifties, wickets, bowlAvg);
    }
  }
})();

console.log("Successfully imported ODI records!");
